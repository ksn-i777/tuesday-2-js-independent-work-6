...
let sortedByName = deepCopyStudents.map(st => ({...st})).sort((a, b) => {
    if(a.name < b.name) {return -1} else {return 1}
});
//console.log(sortedByName);

//5a+. Отсортируйте deepCopyStudents по успеваемости (лучший идёт первым)(sort)
let sortedByScores = deepCopyStudents.map(st => ({...st})).sort((a, b) => {
    if(a.scores > b.scores) {return -1} else {return 1}
});
...
let topStudents = deepCopyStudents.filter(st => ({...st})).sort((a, b) => {
    if(a.scores > b.scores) {return -1} else {return 1}
}).slice(0,3);
...
const addFriends = (students) => {
   const newArr = students.map(st => ({...st, friends: students.filter(s => s.name !== st.name).map(s2 => s2.name)}))
   return newArr
}
...
const getBestStudents = (students, num) => {
    const newArr = students.filter(st => ({...st})).sort((a, b) => {
        if(a.scores > b.scores) {return -1} else {return 1}
    }).slice(0,num)
    return newArr
 }
